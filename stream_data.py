...
# Please configure acquisition parameters in config_quattrocento.py
...
def convert_bytes_to_int(bytes_value, bytes_in_sample):
    ...
    if bytes_in_sample == 2:
        value = struct.unpack('h' * (len(bytes_value) // 2), bytes_value)[0]
...
def create_bin_command(start=1):
    ...
    (start_command,
    number_of_channels,
    sample_frequency) = config_quattrocento.generate_command()
